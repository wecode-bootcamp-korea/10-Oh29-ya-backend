...
class CartView(View):
    ...
    def get(self, request):
        user     = request.user 
        products = user.userorder_set.get(order_status_id=1).orderproduct_set.all().prefetch_related('product__brand','product__image_set')
...
class UpdateView(View):
    ...
    def patch(self, request):
        try:
            ...
            user_order      = UserOrder.objects.select_related('user').get(user=user,order_status=OrderStatus.objects.get(name='장바구니'))
            ...
            if data['cart'][0]["product_list"] == None:
                ...
                for data in data_list:
                    order_product.create(user_order = user_order,product_id = data['id'],quantity = data['quantity'])
            return JsonResponse({'message':'SUCCESS'},status=200)
        except :
...
class DeleteView(View):
    ...
    def delete(self, request):
        try:
            ...
            count         = OrderProduct.objects.filter(user_order = UserOrder.objects.get(user = user)).count()
